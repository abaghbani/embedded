...
void writeRfSettings(RF_SETTINGS * pRfSettings)
{
	...
	WriteReg_CC1101(TI_CC1101_PKTLEN,   pRfSettings->PKTLEN);
}
...
void RFSendPacket(char *txBuffer, char size)
{
  SPIWriteBurstReg(TI_CC1101_TXFIFO, txBuffer, size); // Write TX data
  SPIStrobe(TI_CC1101_STX);           // Change state to TX, initiating
                                            // data transfer
...
char RFReceivePacket(char *rxBuffer, char *length)
{
  ...
  if ((TI_CC_SPIReadStatus(TI_TI_CC1101_RXBYTES) & TI_TI_CC1101_NUM_RXBYTES))
  {
    pktLen = TI_CC_SPIReadReg(TI_TI_CC1101_RXFIFO); // Read length byte

    if (pktLen <= *length)                  // If pktLen size <= rxBuffer
    {
      TI_CC_SPIReadBurstReg(TI_TI_CC1101_RXFIFO, rxBuffer, pktLen); // Pull data
      *length = pktLen;                     // Return the actual size
      TI_CC_SPIReadBurstReg(TI_TI_CC1101_RXFIFO, status, 2);
                                            // Read appended status bytes
      return (char)(status[TI_TI_CC1101_LQI_RX]&TI_TI_CC1101_CRC_OK);
    }                                       // Return CRC_OK bit
    else
    {
      *length = pktLen;                     // Return the large size
      TI_CC_SPIStrobe(TI_TI_CC1101_SFRX);      // Flush RXFIFO
      return 0;                             // Error
    }
  }
  else
